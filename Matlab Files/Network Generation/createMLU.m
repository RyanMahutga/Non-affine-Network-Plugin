...
numEnodes = 50; % number of elastic lamina nodes
numAnodes = 40; % number of actin nodes
...
pC_fibers(idx,:) = [];
[pC_fibers,deg_p] = NetworkPare(pC_fibers, num_nodes,9);
...
% [uA_fibers,deg_a] = NetworkPare(uA_fibers, num_nodes,9);
...
for p = 1:length(uA_fibers)
    ...
    if nodes_All(node1,3)*nodes_All(node2,3) < 0 && uA_fibers(p,5) == 0 % z-dim have opposite signs (i.e. crosses x,y plane at z=0
        cross_idx(j) = p;
        [ x,y ] = XYintercept( nodes_All(node1,:), real_coord2,3) ;
...
[ fib_radsX, init_lens ] = networkFeatures( nodes, fibers, mu, sigma);

fib_rads=[];
for p=1:length(fib_radsX)
    if fib_type(p) == 1
        fib_rads(p) = 3*fib_radsX(p);
    elseif fib_type(p) == 2
        fib_rads(p) = 2*fib_radsX(p);
    else
         fib_rads(p) = fib_radsX(p);
    end
end

init_lens(1:len1) = lens;
fib_vol1=0; fib_vol2=0; fib_vol3=0;
for f=1:length(fibers)
    if fib_type(f)==1
        fib_vol1 = fib_vol1 + scale*init_lens(f)*pi()*fib_rads(f)^2;
    elseif fib_type(f)==2
        fib_vol2 = fib_vol2 + scale*init_lens(f)*pi()*fib_rads(f)^2;
    elseif fib_type(f)==3
        fib_vol3 = fib_vol3 + + scale*init_lens(f)*pi()*fib_rads(f)^2;
    end
end

phi(1) = fib_vol1/scale^3;
phi(2) = fib_vol2/scale^3;
phi(3) = fib_vol3/scale^3;
